...
void PrintedLinkedList(linkedlist_t* list,int n){
...
int len(linkedlist_t *list){
    // if ll is empty then length is 0
    if (list->head == NULL) return 0;
    int length = 0;
    // if the next node points to head then exit the loop
    node_t *trav = list->head;
    node_t *check = list->head;
    while (trav->next != check ){
        trav = trav->next;
        length++;
    }
    return length;
